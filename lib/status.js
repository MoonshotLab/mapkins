...
function getStepperFromMapkinsLeft(mapkinsLeft) {
  // steppers start at 1 at the top, going up going down ([1, 2, 3, 4] top to bottom)
  const dispensed = config.totalMapkins - mapkinsLeft;
  let unaccountedDispensed = dispensed;
  let currentStepper = 0;

  while (currentStepper <= config.numRows) {
    const rowCapacity = config.mapkinsCapacity[currentStepper];
    unaccountedDispensed -= rowCapacity;
    if (unaccountedDispensed < 0) break;
    currentStepper++;
  }

  currentStepper++; // we want steppers to be 1-indexed
  return currentStepper <= config.numRows ? currentStepper : null;
}
...
function asyncGetCurrentStepper() {
  return new Promise((resolve, reject) => {
    asyncGetMapkinsLeft()
      .then(mapkinsLeft => {
        const currentStepper = getStepperFromMapkinsLeft(mapkinsLeft);
...
function asyncGetMapkinsLeft() {
  return new Promise((resolve, reject) => {
    asyncGetMapkinsLeftObj()
      .then(mapkinsLeftObj => {
        const mapkinsLeft = mapkinsLeftObj.val;
        if (mapkinsLeft > config.totalMapkins || mapkinsLeft < 0) {
          reject(new Error('invalid_mapkins_count'));
        } else {
