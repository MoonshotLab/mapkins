...
function asyncHandleMapkinsOutError(bot, message) {
  return new Promise((resolve, reject) => {
    ...
    dispense.asyncNotifyAdminsOfMapkinsOut().catch(err => {
      reject(err);
    });
    ...
    dispense
      ...
      .catch(err => {
        if (err.message === 'already_on_list') {
          ...
          bot.reply(
            message,
            `You're already on our waitlist and we will let you know when the machine is back in working order. Sorry about that!`
          );
          resolve(false);
        } else {
          reject(err);
...
function asyncDispenseMapkin(bot, message) {
  return new Promise((resolve, reject) => {
    asyncMakeSureUserIsNew(message.user)
      .then(() => {
        return dispense.asyncMakeSureThereAreMapkinsLeft();
      })
      .then(() => {
